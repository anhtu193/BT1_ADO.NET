...
class Program
{
    ...
    {
        List<ConNguoi> listConNguoi = new List<ConNguoi>();
        ...
        while (tiepTuc) 
        {
            Console.WriteLine("Nhap thong tin cho doi tuong thu " + (stt + 1));
            ...
            Console.Write("Ban co muon tiep tuc nhap cac doi tuong moi khong? (y/n): ");
            string tiepTucNhap = Console.ReadLine().ToLower();  
            tiepTuc = tiepTucNhap == "y";
            stt++;
        }

        Console.WriteLine("\nDANH SACH CAC DOI TUONG DA NHAP");
        foreach (ConNguoi nguoi in listConNguoi) 
        {
            Console.WriteLine("------------------------");
            nguoi.InThongTin();
        }
